//! Chartreuse's icons, rendered with resvg from the SVG sources in
//! `assets/icon/` (see its README for the design and every generated file).
//!
//! - The app icon, `app-icon.svg`, in the flavor's accent, packed per platform
//!   at bundle and release time: a macOS `.icns` (an `.iconset` packed with
//!   `iconutil`), a Windows `.ico`, and Linux PNGs in the freedesktop `hicolor`
//!   layout. The Windows and Linux icons up to [`SMALL_ICON_MAX_SIZE`] fill the
//!   image edge to edge without the macOS grid margin and drop shadow.
//! - The icons compiled into the app, checked in under [`GENERATED_DIR`] and
//!   regenerated by `cargo xtask icons`: the macOS status item's template
//!   image (`status-item-template.svg`) and the Windows and Linux tray icons
//!   (the small app icon at tray sizes, per flavor). A test fails while they
//!   are stale.
...
use crate::util::{run, tool, workspace_root, Context, Error, Result};
...
/// The macOS status item's template image source.
const STATUS_ITEM_SVG: &str = include_str!("../../assets/icon/status-item-template.svg");

/// Where the icons compiled into the app are checked in, relative to the
/// workspace root.
pub const GENERATED_DIR: &str = "assets/icon/generated";

/// The macOS status item icon's side in points, rendered at 1x and 2x.
const STATUS_ITEM_POINTS: u32 = 18;

/// The pixel sizes in the Windows tray `.ico`: the small icon at 100%, 125%,
/// 150%, 200%, 250%, and 300% scale.
const TRAY_ICO_SIZES: [u32; 6] = [16, 20, 24, 32, 40, 48];

/// The pixel sizes of the Linux tray PNGs: common panel icon sizes, at 1x and
/// 2x.
const TRAY_PNG_SIZES: [u32; 6] = [16, 22, 24, 32, 48, 64];
...
/// Renders the status item's template image at `size × size` pixels.
pub fn render_status_item(size: u32) -> Result<Image> {
    render_svg(STATUS_ITEM_SVG, None, None, size)
}
...
impl Asset {
    fn encode(&self) -> Result<Vec<u8>> {
        match self {
            Self::Png(image) => encode_png(image),
            Self::Ico(images) => encode_ico(images),
        }
    }
}

/// Every file in [`GENERATED_DIR`], by name.
pub fn generated_assets() -> Result<Vec<(String, Asset)>> {
    let mut assets = vec![
        (
            "status-item-template.png".to_owned(),
            Asset::Png(render_status_item(STATUS_ITEM_POINTS)?),
        ),
        (
            "status-item-template@2x.png".to_owned(),
            Asset::Png(render_status_item(2 * STATUS_ITEM_POINTS)?),
        ),
    ];
    for flavor in [Flavor::Development, Flavor::Release] {
        ...
        let ico = TRAY_ICO_SIZES
            .into_iter()
            .map(|size| render_small_app_icon(size, accent))
            .collect::<Result<Vec<_>>>()?;
        assets.push((format!("tray-{name}.ico"), Asset::Ico(ico)));
        for size in TRAY_PNG_SIZES {
            assets.push((
                format!("tray-{name}-{size}.png"),
                Asset::Png(render_small_app_icon(size, accent)?),
            ));
        }
    }
    Ok(assets)
...
/// `cargo xtask icons`: rewrites [`GENERATED_DIR`] from the SVG sources.
pub fn icons() -> Result {
    let dir = workspace_root().join(GENERATED_DIR);
    // Emptied first, so it holds exactly the generated files.
    if dir.exists() {
        std::fs::remove_dir_all(&dir).context(|| format!("removing {}", dir.display()))?;
    }
    let assets = generated_assets()?;
    for (name, asset) in &assets {
        write_file(&dir.join(name), &asset.encode()?)?;
    }
    eprintln!("wrote {} icons to {}", assets.len(), dir.display());
    Ok(())
}
...
mod tests {
    ...
    /// Whether two images match up to rendering differences between
    /// platforms: within 2 in alpha and in premultiplied color.
    ...
    #[test]
    fn status_item_is_a_crisp_black_template() {
        for scale in [1, 2] {
            ...
            // At (points): a selection corner, the crosshair, the gap between
            // them, and the margin.
            let alpha = |x: u32, y: u32| pixel(&image, x * scale, y * scale)[3];
            assert_eq!(alpha(2, 2), 255, "{size}: the top-left corner");
            assert_eq!(alpha(8, 8), 255, "{size}: the crosshair");
            assert_eq!(alpha(5, 5), 0, "{size}: the gap");
            assert_eq!(alpha(0, 0), 0, "{size}: the margin");
        }
    }

    #[test]
    fn checked_in_icons_match_their_sources() {
        let stale = "is stale: run `cargo xtask icons`";
        let dir = workspace_root().join(GENERATED_DIR);
        let assets = generated_assets().unwrap();

        let mut expected: Vec<&str> = assets.iter().map(|(name, _)| name.as_str()).collect();
        ...
        let mut found: Vec<String> = std::fs::read_dir(&dir)
            ...
            .collect();
        found.sort_unstable();
        assert_eq!(found, expected, "{GENERATED_DIR} {stale}");

        for (name, asset) in &assets {
            let bytes = std::fs::read(dir.join(name)).unwrap();
            let (checked_in, rendered): (Vec<Image>, Vec<&Image>) = match asset {
                Asset::Png(image) => (vec![decode_png(&bytes)], vec![image]),
                Asset::Ico(images) => (
                    decode_ico(&bytes)
                        .into_iter()
                        .map(|(_, image)| image)
            ...
            assert_eq!(checked_in.len(), rendered.len(), "{name} {stale}");
            for (checked_in, rendered) in checked_in.iter().zip(rendered) {
                assert!(
                    same_pixels(checked_in, rendered),
                    "{name} at {}px {stale}",
                    rendered.size
                );
            }
